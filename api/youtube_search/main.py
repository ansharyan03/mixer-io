...
def search_two_songs(payload: SearchTwoPayload):
    ...
    if not song1_title or not song1_artist or not song2_title or not song2_artist:
        raise HTTPException(
            status_code=400,
            detail="Both song1 and song2 must include an officialSong and artist",
        )
    ...
    if not video_results1:
        raise HTTPException(
            status_code=404,
            detail=f"No video results found for song1: {song1_title} {song1_artist}",
        )
    ...
    if not video_results2:
        raise HTTPException(
            status_code=404,
            detail=f"No video results found for song2: {song2_title} {song2_artist}",
        )
